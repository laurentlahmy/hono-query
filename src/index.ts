...
// // type GetQueryOptionsParams = {
// //   keyComplement?: string[];
// // } & Omit<QueryOptions, "queryKey" | "queryFn">;
...
type InferResponseType<T> = T extends () => Promise<
  ClientResponse<infer R, any, any>
>
  ? R
  : never;

export function getQueryKey(fn: () => unknown, keyComplement: string[] = []) {
  const queryKeyString = fn.toString().split(".").slice(1).join(".");
  return [queryKeyString, ...keyComplement].filter(Boolean);
}
...
>(fn: T) {
  return async () => {
    ...
    return res.json() as Promise<InferResponseType<T>>;
  };
}
...
>(
  ...
  options: {
    ...
    QueryOptions<InferResponseType<T>, Error>,
    "queryKey" | "queryFn"
  > = {}
) {
  const { keyComplement = [], ...rest } = options;
  ...
  return {
    ...
    queryFn: getQueryFn(fn),
    ...rest,
  } satisfies QueryOptions<InferResponseType<T>, Error>;
}
